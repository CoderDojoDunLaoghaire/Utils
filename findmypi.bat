...
ping -n 1 -w 500 %prefix%.%n% >%tempfile%
if not %mac% == * ( 
		rem try to find MAC match in ARP table
		for /f "tokens=1 delims= " %%i in ('arp -a ^| find /i "%mac%"') do set ip=%%i
		if "%ip%" neq "" echo found your Pi at address %ip% && goto end
	) ELSE (
		rem look for a potential IP address on the same network
		findstr /m "Received = 1" %tempfile% >nul
